...
const anyResource = await db.resource.findFirst({ where: { status: "PUBLISHED", authorId: adminRow.id }, select: { id: true, slug: true, likeCount: true, externalUrl: true } });
const creatorResource = await db.resource.findFirst({ where: { status: "PUBLISHED", authorId: creatorRow.id }, orderBy: { createdAt: "asc" }, select: { id: true, externalUrl: true } });
...
// 点赞：开 → DB 行 + 计数 → 关 → 还原
{
  ...
  const like = await db.like.findUnique({ where: { userId_resourceId: { userId: creatorRow.id, resourceId: anyResource.id } } });
  const mid = await db.resource.findUnique({ where: { id: anyResource.id }, select: { likeCount: true } });
  const r2 = await call(creator, "toggleLike", anyResource.id);
  const after = await db.resource.findUnique({ where: { id: anyResource.id }, select: { likeCount: true } });
...
// 收藏：默认夹自动创建 + 移动夹子 + 删除夹子归未分组 + 取消收藏还原
{
  ...
  const fav = await db.favorite.findUnique({ where: { userId_resourceId: { userId: creatorRow.id, resourceId: anyResource.id } } });
  const defCol = await db.collection.findFirst({ where: { ownerId: creatorRow.id, name: "默认收藏" } });
  ok("收藏落默认夹", !!fav && fav.collectionId === defCol.id);

  await call(creator, "createCollection", { __fd: { name: "测试夹子-A" } });
  const col = await db.collection.findFirst({ where: { ownerId: creatorRow.id, name: "测试夹子-A" } });
  ok("新建夹子", !!col);
  await call(creator, "setFavoriteCollection", anyResource.id, col.id);
  const fav2 = await db.favorite.findUnique({ where: { userId_resourceId: { userId: creatorRow.id, resourceId: anyResource.id } } });
  ...
  const fav3 = await db.favorite.findUnique({ where: { userId_resourceId: { userId: creatorRow.id, resourceId: anyResource.id } } });
  ok("删夹子收藏归未分组", fav3.collectionId === null && !(await db.collection.findUnique({ where: { id: col.id } })));
  const favCountBefore = await db.resource.findUnique({ where: { id: anyResource.id }, select: { favoriteCount: true } });
  await call(creator, "toggleFavorite", anyResource.id);
  const favGone = await db.favorite.findUnique({ where: { userId_resourceId: { userId: creatorRow.id, resourceId: anyResource.id } } });
  const favCountAfter = await db.resource.findUnique({ where: { id: anyResource.id }, select: { favoriteCount: true } });
...
// 关注：开 → 通知 → 关
{
  ...
  const f = await db.follow.findUnique({ where: { followerId_followingId: { followerId: creatorRow.id, followingId: adminRow.id } } });
  const note = await db.notification.findFirst({ where: { userId: adminRow.id, actorId: creatorRow.id, type: "FOLLOW" }, orderBy: { createdAt: "desc" } });
...
let testCommentId;
{
  const r = await call(creator, "addComment", {}, { __fd: { resourceId: anyResource.id, content: "行为测试评论——稍后自动删除" } });
  const c = await db.comment.findFirst({ where: { resourceId: anyResource.id, authorId: creatorRow.id, content: { contains: "行为测试评论" } }, orderBy: { createdAt: "desc" } });
  const note = await db.notification.findFirst({ where: { userId: adminRow.id, actorId: creatorRow.id, type: "COMMENT" }, orderBy: { createdAt: "desc" } });
  ok("发评论 + 作者通知", r.result?.ok === true && !!c && !!note, JSON.stringify(r));
  testCommentId = c?.id;
  // 回复楼层
  const rr = await call(creator, "addComment", {}, { __fd: { resourceId: anyResource.id, content: "行为测试回复", parentId: c.id } });
...
let testResourceId;
{
  ...
  const r = await call(creator, "createResource", {}, { __fd: {
    type: "ARTICLE", title: "行为测试文章-勿留", summary: "测试",
    description: "这是一篇行为测试文章的描述内容，超过十个字。",
    categoryId: category.id, tags: "测试标签",
  } });
  const res = await db.resource.findFirst({ where: { title: "行为测试文章-勿留" } });
  ok("trusted 用户直发上架", r.error === "NEXT_REDIRECT" && res?.status === "PUBLISHED", JSON.stringify(r).slice(0, 120));
  ...
  const r2 = await call(demo, "createResource", {}, { __fd: {
    type: "ARTICLE", title: "行为测试文章2-勿留",
    description: "这是第二篇行为测试文章的描述，超过十个字。",
    categoryId: category.id,
  } });
  const res2 = await db.resource.findFirst({ where: { title: "行为测试文章2-勿留" } });
  ok("非 trusted 发布进审核队列", r2.result?.pending === true && res2?.status === "PENDING", JSON.stringify(r2).slice(0, 120));
  const a2 = await call(admin, "approveResource", res2.id);
  const after = await db.resource.findUnique({ where: { id: res2.id }, select: { status: true, publishedAt: true } });
  ok("admin 通过上架", a2.result?.ok === true && after.status === "PUBLISHED" && !!after.publishedAt);
  const modNote = await db.notification.findFirst({ where: { userId: demoRow.id, type: "MODERATION", resourceId: res2.id } });
  ok("作者收到审核通知", !!modNote);
}

console.log("\n[4] 版本管理");
{
  const r = await call(creator, "addVersion", {}, { __fd: {
    resourceId: creatorResource.id, version: "9.9.9", changelog: "行为测试版本", url: "/uploads/files/test.zip",
  } });
  const v = await db.resourceVersion.findFirst({ where: { resourceId: creatorResource.id, version: "9.9.9" } });
  ...
  await db.resource.update({ where: { id: creatorResource.id }, data: { externalUrl: creatorResource.externalUrl } });
  ok("版本测试数据清理", !(await db.resourceVersion.findFirst({ where: { resourceId: creatorResource.id, version: "9.9.9" } })));
}

console.log("\n[5] 举报");
{
  const r = await call(creator, "reportResource", anyResource.id, "垃圾广告", "行为测试举报");
  const rep = await db.report.findFirst({ where: { reporterId: creatorRow.id, targetResourceId: anyResource.id, status: "OPEN" } });
...
{
  const r1 = await call(creator, "changePassword", {}, { __fd: { current: "test1234", next: "tmp-pass-123", confirm: "tmp-pass-123" } });
  const row1 = await db.user.findUnique({ where: { id: creatorRow.id }, select: { passwordHash: true } });
  const changed = row1.passwordHash !== creatorRow.passwordHash;
  const r2 = await call(creator, "changePassword", {}, { __fd: { current: "tmp-pass-123", next: "test1234", confirm: "test1234" } });
  // 错误旧密码要被拒
  const r3 = await call(creator, "changePassword", {}, { __fd: { current: "wrong-pass", next: "x".repeat(10), confirm: "x".repeat(10) } });
...
{
  const r1 = await call(creator, "changeEmail", {}, { __fd: { email: "creator-tmp@example.com", password: "test1234" } });
  const e1 = await db.user.findUnique({ where: { id: creatorRow.id }, select: { email: true } });
  ok("换邮箱生效", r1.result?.ok === true && e1.email === "creator-tmp@example.com", JSON.stringify(r1));
  // 被占邮箱被拒
  const r2 = await call(creator, "changeEmail", {}, { __fd: { email: "admin@example.com", password: "test1234" } });
  ok("占用邮箱被拒", r2.result?.ok !== true || !!r2.result?.fieldErrors);
  const r3 = await call(creator, "changeEmail", {}, { __fd: { email: "creator@example.com", password: "test1234" } });
  ok("邮箱还原", r3.result?.ok === true);
}

console.log("\n[7] 通知中心动作（用临时测试用户，避免污染真实通知）");
{
  const t = await db.user.create({ data: { email: "notify-tmp@example.com", username: "notify_tmp", passwordHash: "x", name: "notify tmp" } });
  const n1 = await db.notification.create({ data: { userId: t.id, type: "LIKE", resourceId: anyResource.id, actorId: creatorRow.id } });
  const n2 = await db.notification.create({ data: { userId: t.id, type: "SYSTEM", message: "行为测试系统通知" } });

  // 用临时用户登录（passwordHash 是假 hash，走不了 credentials）→ 改用直接构造 action 调用不行，
  // dev-test 路由依赖会话 cookie。改为：临时把密码设为可登录的真实 hash，测完删号。
  // （bcrypt 同步 hash）
  const bcrypt = (await import("bcryptjs")).default;
  await db.user.update({ where: { id: t.id }, data: { passwordHash: bcrypt.hashSync("test1234", 10) } });
...
console.log("\n[8] admin 媒体库（直传 + 删除）");
{
  ...
  const r = await call(admin, "uploadMedia", {}, { __fd: {}, __file: { name: "smoke.png", mime: "image/png", base64: png.toString("base64") } });
  // uploadMediaAction(prev, fd) 的 fd 里文件字段名是 file
  ok("admin 直传图片", r.result?.ok === true, JSON.stringify(r).slice(0, 200));
  const m = await db.media.findFirst({ where: { fileName: "smoke.png" }, orderBy: { createdAt: "desc" } });
  ok("媒体落库", !!m);
  if (m) {
    const d = await call(admin, "deleteMedia", m.id);
    ok("删除未引用媒体", d.result?.ok === true && !(await db.media.findUnique({ where: { id: m.id } })), JSON.stringify(d).slice(0, 200));
  }
  // 被引用的媒体拒绝删除（拿一个正在当封面的媒体）
  const withCover = await db.resource.findFirst({ where: { coverMediaId: { not: null } }, select: { coverMediaId: true } });
  const cover = withCover ? await db.media.findUnique({ where: { id: withCover.coverMediaId } }) : null;
  if (cover) {
...
console.log("\n[9] 封禁/解封（admin action + 审计日志）");
{
  ...
  const audit = await db.auditLog.findFirst({ where: { action: "BAN", targetId: demoRow.id }, orderBy: { createdAt: "desc" } });
...
console.log("\n[10] 清理发布测试资源");
{
  for (const title of ["行为测试文章-勿留", "行为测试文章2-勿留"]) {
    ...
    for (const l of tagLinks) await db.tag.update({ where: { id: l.tagId }, data: { count: { decrement: 1 } } }).catch(() => {});
