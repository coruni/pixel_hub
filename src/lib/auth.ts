...
import { getRuntimeConfig, githubClientId, githubClientSecret } from "./runtime-config";
...
// NextAuth v5 函数式懒初始化：每次请求重新组装配置，GitHub OAuth 凭据从后台运行配置
// 读取（getRuntimeConfig，带 .env 回退）——管理员在后台填写/更新 OAuth App 无需改 env 重启。
export const { handlers, auth, signIn, signOut } = NextAuth(async () => {
  const cfg = await getRuntimeConfig();
  const githubId = githubClientId(cfg);
  const githubSecret = githubClientSecret(cfg);

  return {
    ...authConfig,
    adapter: PrismaAdapter(prisma),
    providers: [
      // GitHub OAuth 仅在后台（或旧 env）配置了凭据时启用
