...
export default function LoginForm({ githubEnabled, callbackUrl = "/" }: { githubEnabled: boolean; callbackUrl?: string }) {
 ...
 {ghError && <p className="mt-4 rounded-none bg-red-50 px-3 py-2 text-sm text-red-600">{ghError}</p>}
 ...
 <span className="h-px flex-1 bg-neutral-200" />或<span className="h-px flex-1 bg-neutral-200" />
 </div>
 ...
 <span className="ml-1">
 <code>admin</code>（管理员·免审） / <code>creator</code>（可信创作者·免审） / <code>demo</code>（普通用户，投稿需审核）
 </span>
 </div>
 </div>
 );
