...
export function PasswordForm() {
 const [state, formAction, pending] = useActionState<SettingsActionState, FormData>(changePasswordAction, {});
 return (
 ...
 <input id="next" name="next" type="password" autoComplete="new-password" className={input} />
...
export function EmailForm({ currentEmail }: { currentEmail: string | null }) {
 const [state, formAction, pending] = useActionState<SettingsActionState, FormData>(changeEmailAction, {});
 return (
 <form action={formAction} className="space-y-4">
 <div>
