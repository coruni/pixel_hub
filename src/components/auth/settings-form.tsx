...
export default function SettingsForm({ name, bio }: { name: string | null; bio: string | null }) {
 ...
 <input id="name" name="name" defaultValue={name ?? ""} maxLength={30} className={input} placeholder="显示名称，留空则用用户名" />
 ...
 />
 {state.fieldErrors?.bio && <p className="mt-1 text-xs text-red-500">{state.fieldErrors.bio[0]}</p>}
 </div>
