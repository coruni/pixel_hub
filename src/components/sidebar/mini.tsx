...
export function MetricText({ item, metric }: { item: FeedItem; metric: MetricKind }) {
  ...
  return (
    <span
      className="flex shrink-0 items-center gap-1 text-[11px] tabular-nums text-neutral-400"
      title={`${label} ${metricOf(item, metric)}`}
    >
...
function RankBadge({ rank, overlay = false }: { rank: number; overlay?: boolean }) {
  ...
  return (
    <span
      className={`grid h-5 w-5 shrink-0 place-items-center rounded-none text-[11px] font-semibold tabular-nums ${
        top
...
export function MiniRow({
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-2.5 rounded-none p-1 transition hover:bg-brand-50"
    >
      {rank != null && <RankBadge rank={rank} />}
      <MiniThumb item={item} className="h-11 w-16 shrink-0 rounded-none" />
      <span className="min-w-0 flex-1">
        <span className="block truncate text-xs font-medium text-neutral-800 group-hover:text-neutral-950">
          {item.title}
        </span>
        <span className="mt-0.5 flex items-center gap-1.5 text-[11px] text-neutral-400">
          ...
          {item.type === "ARTICLE" && (
            <span className="rounded-none bg-sky-50 px-1 text-[9px] font-medium text-sky-600">
              文章
            </span>
          )}
          {metric && (
            <span className="ml-auto">
              <MetricText item={item} metric={metric} />
            </span>
          )}
        </span>
      </span>
...
export function MiniCard({
  ...
  return (
    <Link
      href={`/resources/${item.slug}`}
      className="group block overflow-hidden rounded-none border border-brand-200/70 transition hover:border-brand-500"
    >
