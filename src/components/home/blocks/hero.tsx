...
import { ArrowUpRight, Download, Heart, Image as ImageIcon, Gamepad2, Newspaper } from "lucide-react";
...
export default async function HeroBlock({ title, cfg }: { title: string | null; cfg: { featuredIds: string[] } }) {
      ...
      <div className={frame}>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            <div className="absolute left-3 top-3"><TypeBadge type={big.type} /></div>
            ...
            <div className="absolute inset-x-0 bottom-0 flex flex-wrap items-end justify-between gap-3 p-5">
              <div className="min-w-0">
                ...
                {big.summary && <p className="mt-1 hidden max-w-xl truncate text-sm text-white/80 sm:block">{big.summary}</p>}
                <p className="mt-2 flex items-center gap-2 text-xs text-white/75">
                  ...
                  <span className="inline-flex items-center gap-1"><Heart size={12} />{formatCount(big.likeCount)}</span>
                  {big.type === "GAME" && (
                    <span className="inline-flex items-center gap-1"><Download size={12} />{formatCount(big.downloadCount)}</span>
                  )}
                </p>
              </div>
