...
export default function DetailTwocol({ ctx }: { ctx: DetailCtx }) {
 ...
 <>
 {" · "}
 <Link href={`/browse?cat=${detail.category.slug}`} className="hover:text-neutral-800 hover:underline">
 ...
 {detail.summary && <p className="mt-1.5 max-w-2xl text-sm leading-6 text-neutral-500">{detail.summary}</p>}
 </div>
 </div>
