...
export function VersionForm({ resourceId }: { resourceId: string }) {
 ...
 const [state, formAction, pending] = useActionState<ResourceActionState, FormData>(addVersionAction, {});
 ...
 {open && (
 <form action={formAction} className="mt-3 space-y-3 rounded-none border border-brand-200 bg-surface p-4">
 ...
 <input type="file" hidden disabled={attUploading} onChange={(e) => onAttachment(e.target.files?.[0] ?? null)} />
 </label>
 {url.startsWith("/") && <span className="text-xs text-emerald-600">✓ 已上传站内附件</span>}
