...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const resIds = [
    ...new Set(
      rows
        .filter((r) => r.type === "RESOURCE" && r.targetResourceId)
        .map((r) => r.targetResourceId!),
    ),
  ];
  ...
  const uidTargets = [
    ...new Set(rows.filter((r) => r.type === "USER" && r.targetUserId).map((r) => r.targetUserId!)),
  ];
  const userMap = new Map(
    (uidTargets.length
      ? await prisma.user.findMany({
          where: { id: { in: uidTargets } },
  ...
  const describe = (r: Row): { href?: string; text: string } => {
    if (r.type === "RESOURCE" && r.targetResourceId) {
      ...
      return res
        ? { href: `/resources/${res.slug}`, text: `「${res.title}」` }
        : { text: "资源已不存在" };
    }
      ...
      {list.length === 0 ? (
        ...
        <ul className="space-y-3">
          {list.map((group) => {
            ...
            const actionsId =
              first.targetResourceId ?? first.targetCommentId ?? first.targetUserId ?? null;
            return (
              <li
                key={keyOf(first)}
                className="rounded-none border border-brand-200 bg-surface p-4"
              >
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  <span className={`rounded-none px-1.5 py-0.5 text-[10px] font-medium ${st.cls}`}>
                    {st.text}
                  </span>
