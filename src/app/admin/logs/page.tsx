...
export default async function LogsPage() {
 ...
 (adminIds.length
 ? await prisma.user.findMany({ where: { id: { in: adminIds } }, select: { id: true, username: true, name: true } })
 : []
 ...
 <td className="whitespace-nowrap px-4 py-2.5 text-xs text-neutral-400">{timeAgo(r.createdAt)}</td>
 ...
 {r.targetId ? <span className="text-neutral-300"> #{r.targetId.slice(-8)}</span> : ""}
 </td>
 <td className="max-w-[280px] truncate px-4 py-2.5 text-xs text-neutral-500">{r.note ?? "—"}</td>
 </tr>
 );
 })}
 </tbody>
