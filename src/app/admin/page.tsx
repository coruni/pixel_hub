...
export default async function AdminIndex() {
 ...
 const days: { key: string; label: string; resources: number; users: number; pv: number; ips: number }[] = [];
 for (let i = 6; i >= 0; i--) {
 const d = new Date();
 d.setHours(0, 0, 0, 0);
 ...
 <div className={`h-full ${t.cls}`} style={{ width: `${Math.round((t.v / total) * 100)}%` }} />
 </div>
 </div>
 );
 })}
 ...
 style={{ height: `${Math.max((d.resources / trendMax) * 100, d.resources > 0 ? 6 : 2)}%` }}
 />
 <div
 title={`新用户 ${d.users}`}
 className="w-2.5 bg-neutral-400"
 style={{ height: `${Math.max((d.users / trendMax) * 100, d.users > 0 ? 6 : 2)}%` }}
 />
 </div>
