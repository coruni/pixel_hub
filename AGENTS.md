...
验证只做两件事：**类型检测**和**代码质量检测**。不跑构建，不起开发服务器，不做运行时截图验收。

- 每次改动收尾固定执行 `npx tsc --noEmit` 和 `npm run lint`，两条都必须干净；有新增报错先修掉再交付。
- 文档或配置修改：追加 `git diff --check`，检查所有引用路径和命令真实存在，并确认无无关文件进入 diff。
- 不运行 `npm run build`，不为此启动 `next dev`，也不要求用真实页面截图作为视觉验收依据；涉及视觉的改动以源码、设计令牌和组件复用为准，并在交付说明里写明未做运行时验证。
- 不主动运行 `npm run test:smoke` 等会写数据库的脚本；仅在用户明确要求时执行，且先确认使用隔离测试数据和可恢复策略。
- 不为了验证而在 `src/app` 下新增临时路由或页面；确有必要时用完必须删除，且不得留在交付 diff 里（`.gitignore` 之外的临时文件同样算污染）。
