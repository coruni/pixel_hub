  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      async function addMedia(
        ...
        return tx.media.create({
          data: {
            ...
            width: 1500,
            height: 1000,
            size: 900 + (fileIdx.get(file) ?? 0) * 100,
            mime: "image/svg+xml",
            sort,
      ...
      async function createResource(o: {
        ...
        const row = await tx.resource.create({
          data: {
            ...
            publishedAt:
              o.status === "PUBLISHED" ? new Date(Date.now() - (o.counts?.view ?? 1) * 1000) : null,
          },
        });
        return row;
      }
      ...
      const img1 = await createResource({
        ...
        type: "IMAGE",
        status: "PUBLISHED",
        slug: "minimal-wallpaper-collection",
        title: "四季色调 · 极简壁纸合辑",
      ...
      const art1 = await createResource({
        ...
        type: "ARTICLE",
        status: "PUBLISHED",
        slug: "pixel-art-beginner-guide",
        title: "像素画入门：从 16×16 小图开始",
      ...
      await tx.favorite.create({
        data: { userId: demo.id, resourceId: img1.id, collectionId: favCollection.id },
      });
      await tx.favorite.create({
        data: { userId: demo.id, resourceId: img2.id, collectionId: favCollection.id },
      });
